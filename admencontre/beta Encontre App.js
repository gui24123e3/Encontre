...
// Função para abrir o WhatsApp com informações do comércio
const abrirWhatsAppComInformacoes = () => {
  const numero = '16994392545';
  const motivo = "Motivo da denúncia: \n\n"; // Texto inicial para o motivo
  const data = "Data do ocorrido: \n\n"; // Texto inicial para a data
  const informacoesComercio = `
Nome: ${item.nome}
Cidade: ${item.cidade}
Estado: ${item.estado}
...
`;
  ...
  const mensagem = `${informacoesComercio}\n${motivo}${data}`;
...
<View style={styles.iconContainer}>
            <Icon 
                name="warning" // Ícone de alerta
                size={27} 
                color="red" 
                onPress={abrirWhatsAppComInformacoes}
                style={{ marginLeft: -10 }} 
            />
        </View>
                </View>
